...
const TimeSlotPicker: React.FC<TimeSlotPickerProps> = ({ options, selected, onSelect }) => {
  const slotIcons: Record<string, JSX.Element> = {
    morning: (
      <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
    ),
    afternoonEvening: (
    ...
    night: (
      <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>
    ),
    fullday: (
      <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    ),
  };
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center p-2 sm:px-4 sm:py-3 rounded-xl transition-all duration-200 focus:outline-none flex-1 min-w-[calc(50%-8px)] sm:min-w-[120px] max-w-[180px]
                ${isSelected
                  ? 'bg-primary border-2 border-primary shadow-md scale-[1.03] text-primary-foreground' : 'bg-card border border-border hover:bg-secondary/80 hover:border-accent hover:scale-[1.01] text-muted-foreground'}
              `}
