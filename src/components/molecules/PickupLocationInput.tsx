    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-3 mb-4 w-full">
        <PickupOption
          ...
          label="Pick me up at the park gate"
          description="Your driver will be waiting at the entrance."
      ...
      {pickupMode === 'custom' && (
        <div className="relative w-full">
          <input
            ...
            className="border-2 border-input rounded-lg p-3 w-full focus:outline-none focus:ring-2 focus:ring-ring text-lg text-foreground placeholder-muted-foreground bg-card shadow-sm"
            autoComplete="off"
          />
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute left-0 right-0 mt-1 z-20 bg-popover border border-border w-full rounded shadow overflow-hidden">
              {loadingSuggestions && <div className="p-2 text-muted-foreground">Loading...</div>}
              {suggestions.map((suggestion) => {
                const className = 'p-2 text-foreground cursor-pointer hover:bg-accent/10 hover:text-accent-foreground';
                return (
                  <div
