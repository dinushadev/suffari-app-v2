...
const PickupOption: React.FC<PickupOptionProps> = ({ icon, label, description, checked, onChange, value, className = '' }) => {
  return (
    <label className={`flex items-center gap-3 px-3 py-3 rounded-lg cursor-pointer border-2 transition-colors min-w-0 w-full ${checked ? 'border-primary bg-primary/10' : 'border-input bg-card hover:border-accent'} ${className}`}
      style={{ minWidth: 0 }}
    >
      ...
      <span className="flex flex-col min-w-0 flex-1 w-full text-left">
        ...
        {description && <span className="text-xs text-muted-foreground mt-1 whitespace-normal break-words">{description}</span>}
      </span>
      <input
        type="radio"
        checked={checked}
