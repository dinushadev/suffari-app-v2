  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-background p-4">
      ...
      <div className="flex gap-4 mb-6">
        <button
          className={`px-4 py-2 rounded-t font-semibold border-b-2 ${tab === 'signin' ? 'border-orange text-orange' : 'border-transparent text-gray-500'}`}
          onClick={() => { setTab('signin'); setError(""); }}
        >
          Sign In
        </button>
        <button
          className={`px-4 py-2 rounded-t font-semibold border-b-2 ${tab === 'signup' ? 'border-orange text-orange' : 'border-transparent text-gray-500'}`}
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full max-w-xs mb-4 border border-gray-300 py-2 rounded font-semibold bg-white hover:bg-gray-50 disabled:opacity-50"
